...
#include "obj_dir_Parallel/VParallel.h"
#include "stdio.h"
#include "stdint.h"
#include "verilated.h"
...
#if VLINETRACE
#include "obj_dir_Parallel/VParallel__Syms.h"
#include "svdpi.h"
#endif
...
VParallel_t * create_model( const char *vcd_filename ) {
  ...
  m     = (VParallel_t *) malloc( sizeof(VParallel_t) );
  model = new VParallel();
  ...
  m->EN = &model->EN;
  m->OUTPUTA = &model->OUTPUTA;
  m->OUTPUTB = &model->OUTPUTB;
  m->clk = &model->clk;
  m->dec_out = &model->dec_out;
  m->dec_select = &model->dec_select;
  m->dta = &model->dta;
  m->rec_rdy = &model->rec_rdy;
  m->rec_val = &model->rec_val;
  m->reset = &model->reset;
...
void destroy_model( VParallel_t * m ) {

  #if VM_COVERAGE
    VerilatedCov::write( "coverage.dat" );
  #endif

  VParallel * model = (VParallel *) m->model;
...
#if VLINETRACE
void trace( VParallel_t * m, char* str ) {
  ...
  svSetScope( &model->__VlSymsp->__Vscope_Parallel__v );
